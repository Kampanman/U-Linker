...
# 定数メッセージ
MSG_PROMPT_TXT_FOLDER = "更新するulinker_notesの行データ（txt）が格納されているフォルダを選択してください。"
...
MSG_PROMPT_CSV_FILE = "更新する行データを反映するulinker_notesのCSVを選択してください。"
MSG_NOT_CSV = "選択されたファイルはCSVではありません。"
MSG_NOT_ULINKER_NOTES_CSV = "選択されたファイルはulinker_notesのCSVではありません。"
...
def main():
  ...
  try:
    ...
    for txt_file_info in potential_txt_files_info:
      ...
      try:
        with open(file_path, 'r', encoding='utf-8') as f:
          ...
          # 「csv-row_」が名称に含まれる.txtファイルの内容における、半角カンマ（,）の個数が10ではない場合
    ...
    # ユーザーが選択したCSVが「ulinker_notes」を名称に含む.csvファイルではない場合
    if "ulinker_notes" not in os.path.basename(csv_file_path).lower():
      print(MSG_NOT_ULINKER_NOTES_CSV)
      return
    ...
    # CSVの内容において、半角カンマ（,）が10個ではない行が存在する場合
