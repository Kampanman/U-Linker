...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['token']) && isAlphanumeric16($_POST['token'])) {
    ...
    if (isset($_POST['ownerId']) && isset($_POST['type']) && $_POST['type'] === 'getBookmarksList') {
      $bookmarkSql = "SELECT `contents_id`, `title`, `url`, `created_at` ".
                    "FROM " . BOOKMARK_TABLE . " WHERE created_user_id = :owner_id";
      $stmt = $connection->prepare($bookmarkSql);
      ...
      if ($bookmarks) {
        ...
        $res['message'] = '該当するサイトが見つかりませんでした。';
      }
    } else {
